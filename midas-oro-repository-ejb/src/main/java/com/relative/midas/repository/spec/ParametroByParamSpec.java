...
public class ParametroByParamSpec extends AbstractSpecification<TbMiParametro> {
	...
	public ParametroByParamSpec(String nombre, String tipo, EstadoMidasEnum estado, String caracteriticaUno, String caracteristicaDos) {
		this.nombre = nombre;
		this.tipo=tipo;
		this.estado=estado;
	...
	public Predicate toPredicate(Root<TbMiParametro> poll, CriteriaBuilder cb) {
		...
		if(this.caracteriticaUno != null && !this.caracteriticaUno.isEmpty()) {
			patientLevelPredicates.add(cb.like(poll.<String>get("caracteriticaUno"), "%" + this.caracteriticaUno + "%"));
		}
		if(this.caracteristicaDos != null && !this.caracteristicaDos.isEmpty()) {
			patientLevelPredicates.add(cb.like(poll.<String>get("caracteristicaDos"), "%" + this.caracteristicaDos + "%"));
		}
