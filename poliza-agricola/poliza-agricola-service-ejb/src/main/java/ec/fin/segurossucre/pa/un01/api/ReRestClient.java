...
public class ReRestClient<T> {
	...
	public static <T> Map<String, Object> callRestApi(String contentTypeAccept, String contentType, String athorization, String contentAsString, String method,
		...
		try {
			...
			RestClientWrapper cw= new RestClientWrapper();
			cw.setAcceptCharset( SiniestroAgricolaConstantes.BPMS_REST_DEFAULT_CHARSET );
		...
		try {
			...
			if (responseCode >= 200 && responseCode < 300) {
				...
				if(StringUtils.isNotBlank(responseBody)) {
					...
					UN01ColeccionResponseWrapper responseWrapper = gson.fromJson(responseBody, UN01ColeccionResponseWrapper.class);
					results.put(ReRestClient.RETURN_OBJECT, responseWrapper);
				}
