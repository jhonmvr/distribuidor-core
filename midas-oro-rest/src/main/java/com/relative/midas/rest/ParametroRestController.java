...
import com.relative.midas.enums.PaisesEnum;
...
import com.relative.midas.wrapper.YearMonthDay;
...
public class ParametroRestController extends BaseRestController implements CrudRestControllerInterface<TbMiParametro, GenericWrapper<TbMiParametro>> {
	...
	private PaginatedListWrapper<TbMiParametro> findByParams(String nombre, String tipo, String estado, String caracteristicaUno,
		...
		List<TbMiParametro> actions = this.sas.findParametroByParam(nombre, tipo, 
				estado != null && !estado.isEmpty()?MidasOroUtil.getEnumFromString(EstadoMidasEnum.class, estado):null
	...
	@ApiOperation(
		value = "GenericWrapper<YearMonthDay>", 
		notes = "Retorna la diferencia entre fecha inicio y fecha actual",
		response = GenericWrapper.class)
	public  GenericWrapper<YearMonthDay> getDiffBetweenDateInicioActual(
		...
		GenericWrapper<YearMonthDay> w = new GenericWrapper<>();
		w.setEntidad(MidasOroUtil.countDaysBetweenDates(
				MidasOroUtil.formatSringToDate(fechaInicio, formato)));
		return w;
	}
